...
export default class TodoScreen extends Component {
  ...
  editTask = (taskIndex)=>{
    const filteredResult = this.state.data.filter((item, index)=> index == taskIndex);
    let val = filteredResult[0].text;

    this.setState({
      editText: val, editId: taskIndex, 
      modalVisible: true
    });
  }
  updateTask = ()=>{
    let pos =  this.state.editId;
  ...
  deleteTask = (taskIndex)=>{
    ...
    const task = this.state.data.filter((item, index)=> index != taskIndex);
                    ...
                    return(
                      <View key={item.id}  style={styles.taskContainer}>
                        ...
                        <View style={styles.editContainer}>
                            <TouchableOpacity onPress={()=>this.editTask(index)} >
